...
def main():
    ...
    all_probs = np.concatenate(all_probs, axis=0)  # shape [366, 5]

    # Save for bootstrap
    np.save(os.path.join(C.OUTPUT_DIR, "test_labels.npy"), np.array(all_labels))
    np.save(os.path.join(C.OUTPUT_DIR, "test_preds.npy"),  np.array(all_preds))
    np.save(os.path.join(C.OUTPUT_DIR, "test_probs.npy"),  all_probs)
    print("  Saved predictions and probabilities for bootstrap CI.")
